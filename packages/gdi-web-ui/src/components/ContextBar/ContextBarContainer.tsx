...
export function ContextBarContainer(props: ContextBarProps) {
    ...
    const onAction = useCallback(
        (action: Action) => {
            ...
            const contextBarItemDefinition = contextBarItems.find(
                (item) => item.id === contextBarItemId
            );
            ...
            if (existingWidget) {
                ...
                dispatch(
                    actions.patch(existingWidget.id, { state: nextState })
                );
                return;
            }
    ...
    useAction('ADD_ITEM_TO_CONTEXT_BAR', onAction);
