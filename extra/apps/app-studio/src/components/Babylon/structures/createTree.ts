...
var createBranch = function (
    ...
    for (var d = 0; d < branchSlices; d++) {
        ...
        _core_point.addInPlace(
            branchSys.x.scale(
                bowHeight *
                    Math.exp(-_d_slices_length) *
                    Math.sin(bowFreq * _d_slices_length * Math.PI)
            )
        );
        ...
        _xsr =
            branchRadius *
            (1 + (0.4 * Math.random() - 0.2)) *
            (1 - (1 - branchTaper) * _d_slices_length);
        _radii.push(_xsr);
        ...
        for (var a = 0; a < _a_sides; a++) {
            ...
            _path = branchSys.x
                .scale(_xsr * Math.cos(_theta))
                .add(branchSys.z.scale(_xsr * Math.sin(_theta)));
            //align with core
    ...
    var _branch = BABYLON.MeshBuilder.CreateRibbon(
        'branch',
        { pathArray: _cross_section_paths, closeArray: true },
        scene
    );
...
var createTreeBase = function (
    ...
    for (var f = 0; f < forks; f++) {
        ...
        _fork_branch_direction = _trunk_sys.y
            ...
            .add(
                _trunk_sys.x.scale(
                    Math.sin(randPct(forkAngle, 0.15)) * Math.sin(_turn)
                )
            )
            .add(
                _trunk_sys.z.scale(
                    Math.sin(randPct(forkAngle, 0.15)) * Math.cos(_turn)
                )
            );
        ...
        if (boughs > 1) {
            ...
            for (var k = 0; k < forks; k++) {
                ...
                _bough_direction = _fork_branchSys.y
                    .scale(Math.cos(randPct(forkAngle, 0.15)))
                    .add(
                        _fork_branchSys.x.scale(
                            Math.sin(randPct(forkAngle, 0.15)) *
                                Math.sin(_bough_turn)
                        )
                    )
                    .add(
                        _fork_branchSys.z.scale(
                            Math.sin(randPct(forkAngle, 0.15)) *
                                Math.cos(_bough_turn)
                        )
                    );
...
export var createTree = function (
    ...
    var _leaf = BABYLON.MeshBuilder.CreateDisc(
        'leaf',
        {
    ...
    var _set_leaves = function (particle: any, i: any, s: any) {
        ...
        particle.position = new BABYLON.Vector3(
            _px,
            _py +
                ((0.6 * _leaf_width) / leafWHRatio + _base.radii[_a][_upper]) *
                    (2 * (s % 2) - 1),
            _pz
        );
    ...
    var _mini_leaves_SPS = new BABYLON.SolidParticleSystem(
        'minileavesSPS',
        scene,
        { updatable: false }
    );
    ...
    for (var f = 0; f < Math.pow(forks, boughs + 1); f++) {
        _turns.push(
            randPct(Math.floor(f / Math.pow(forks, boughs)) * _fork_turn, 0.2)
        );
    }
    ...
    var _set_mini_trees = function (particle: any, i: any, s: any) {
        ...
        var mini_top = new BABYLON.Vector3(
        ...
        var _mini_direction = _mini_sys.y
            ...
            .add(
                _mini_sys.x.scale(
                    Math.sin(randPct(forkAngle, 0)) * Math.sin(turn)
                )
            )
            .add(
                _mini_sys.z.scale(
                    Math.sin(randPct(forkAngle, 0)) * Math.cos(turn)
                )
            );
        var axis = BABYLON.Vector3.Cross(BABYLON.Axis.Y, _mini_direction);
        var _theta = Math.acos(
            BABYLON.Vector3.Dot(_mini_direction, BABYLON.Axis.Y) /
    ...
    for (var b = 0; b < branches; b++) {
        ...
        _places.push([
            Math.floor(Math.random() * _bplen),
            Math.floor(Math.random() * (_bp0len - 1) + 1),
        ]);
    }
    ...
    var _set_branches = function (particle: any, i: any, s: any) {
        ...
        var _b = _places[s][1];
        var _mini_sys = _base.directions[_a];
        var _mini_place = new BABYLON.Vector3(
            _base.paths[_a][_b].x,
            _base.paths[_a][_b].y,
            _base.paths[_a][_b].z
        );
        ...
        var _mini_direction = _mini_sys.y
            ...
            .add(
                _mini_sys.x.scale(
                    Math.sin(randPct(branchAngle, 0)) * Math.sin(_turn)
                )
            )
            .add(
                _mini_sys.z.scale(
                    Math.sin(randPct(branchAngle, 0)) * Math.cos(_turn)
                )
            );
