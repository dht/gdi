...
export const initSap = (
    ...
    storeBuilder
        .withReducers(APP_ID, googleSync.reducers)
        .withInitialState(APP_ID, googleSync.initialState)
        .withSagas(...appSagas)
        .withPostBuildHook(googleSync.clearState);

    i18nBuilder //
        .withKeysByLanguage(APP_ID, i18n);

    selectorsBuilder //
        .withSelectors(APP_ID, googleSync.selectors);

    apiConfigBuilder //
        .withEndpointsConfigOverrides(
            googleSync.endpointsConfigOverrides(connectionType) as any
        );
