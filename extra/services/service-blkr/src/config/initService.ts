...
export const initSap = (
    ...
    storeBuilder
        .withReducers(APP_ID, blkr.reducers)
        .withInitialState(APP_ID, blkr.initialState)
        .withSagas(...appSagas)
        .withPostBuildHook(blkr.clearState);

    i18nBuilder //
        .withKeysByLanguage(APP_ID, i18n);

    selectorsBuilder //
        .withSelectors(APP_ID, blkr.selectors);

    apiConfigBuilder //
        .withEndpointsConfigOverrides(
            blkr.endpointsConfigOverrides(connectionType) as any
        );
