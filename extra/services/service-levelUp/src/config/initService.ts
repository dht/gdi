...
export const initSap = (
    ...
    storeBuilder
        .withReducers(APP_ID, levelUp.reducers)
        .withInitialState(APP_ID, levelUp.initialState)
        .withSagas(...appSagas)
        .withPostBuildHook(levelUp.clearState);

    i18nBuilder //
        .withKeysByLanguage(APP_ID, i18n);

    selectorsBuilder //
        .withSelectors(APP_ID, levelUp.selectors);

    apiConfigBuilder //
        .withEndpointsConfigOverrides(
            levelUp.endpointsConfigOverrides(connectionType) as any
        );
